...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        for i in range(n_sample):
            ...
            sigma_path = [np.exp(-0.5* self.alpha**2 *delta_t + self.alpha * np.sqrt(delta_t) * z[i, t]) for t in range(n_step)]
            sigma_path.insert(0, sigma)
            sigma_path = np.cumprod(sigma_path)
            ...
            new_sigma = np.sqrt((1-self.rho ** 2) * I / texp)
            price_array[:, i] = self.normal_model.price(strike, new_spot, texp, new_sigma, cp_sign)  # replace the jth column
